...
def manage_services():
    if request.method == 'POST':
        try:
            # Getting data from the "Add New Service" modal
            ...
            # Create and save new service to database
            new_service = Service(
                service_id=s_id,
                service_name=s_name,
                description=s_desc,
    ...
    # GET request: Fetch all services to populate the dropdowns in services.html
    all_services = Service.query.filter_by(is_active=True).order_by(Service.service_name).all()
