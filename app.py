...
# A. SQLAlchemy Config
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://root:Megha%402004@localhost/invoice_generator?charset=utf8mb4'
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            new_inv = Invoice(
                ...
                invoice_date=datetime.datetime.strptime(request.form.get('invoice_date'), '%Y-%m-%d'),
...
def edit_invoice(id):
    ...
    if request.method == 'POST':
        try:
            ...
            invoice.invoice_date = datetime.datetime.strptime(request.form.get('invoice_date'), '%Y-%m-%d')
            ...
            if request.form.get('amount_received'):
                invoice.amount_received = float(request.form.get('amount_received'))
...
def contract_invoice(id=None):
    ...
    for c in contracts:
        ...
        # Fetch Primary Service for this contract
        # Note: If a contract has multiple services, you might want to fetch all and pass as JSON
    ...
    if request.method == 'POST':
        try:
            ...
            # 2. Update Invoice Items (Remove old, add new)
            # We flush here to get the invoice.id if it's a new record
            db.session.flush()
            InvoiceItem.query.filter_by(invoice_id=invoice.id).delete()
            ...
            for i in range(len(serv_list)):
                amount_val = float(amt_list[i]) if amt_list[i] else 0.0
                if amount_val > 0:
                    item = InvoiceItem(
                        invoice_id=invoice.id,
                        contract_id=invoice.contract_id,
                        service_id=int(serv_list[i]) if serv_list[i] else None,
                        amount=amount_val
                    )
                    running_subtotal += amount_val
                    db.session.add(item)

            # 3. Final Calculations (Server-side validation)
            # Ensure the total amount matches what was calculated on the frontend
            tax_rate = 0.18 if invoice.tax_type in ['GST', 'IGST'] else 0.0
            invoice.tax_amount = running_subtotal * tax_rate
            invoice.total_amount = running_subtotal + invoice.tax_amount
...
def list_customers():
    ...
    month = request.args.get('month', '')  # Added
    year = request.args.get('year', '')    # Added
    ...
    customers = cursor.fetchall() # Using standard fetchall for dictionary=True
    conn.close()
    
    # 4. Return variables to template so dropdowns "remember" choices
    return render_template('customer_list.html', 
                           customers=customers, 
                           search_term=search, 
                           country_filter=country, 
                           filter_month=month,  # Passed back
                           filter_year=year,    # Passed back
                           sort_by=sort_by)
    ...
    if request.method == 'POST':
        ...
        cursor.execute("""
            INSERT INTO customers (
                ...
                city, state, pincode, country, deal_owner, gst_status, gst_type, gst_number, 
        ...
        """, (
            ...
            request.form['country'], request.form.get('deal_owner'), status, tax_type, 
            request.form.get('gst_number'), request.form['currency'], j_date, 
            request.form.get('notes'), request.form.get('tax_disclaimer')
        ))
    ...
    if request.method == 'POST':
        ...
        cursor.execute("""
            UPDATE customers SET company_name=%s, first_name=%s, last_name=%s, email=%s, phone=%s, 
                address=%s, city=%s, state=%s, pincode=%s, country=%s, deal_owner=%s, gst_status=%s, 
        ...
        """, (
            request.form['company_name'], request.form.get('first_name'), request.form.get('last_name'),
            request.form.get('email'), request.form.get('phone'), request.form.get('address'),
            request.form.get('city'), request.form.get('state'), request.form.get('pincode'), # Added pincode
            request.form['country'], request.form.get('deal_owner'), status, tax_type, 
            request.form.get('gst_number'), request.form['currency'], j_date, 
            request.form.get('notes'), request.form.get('tax_disclaimer'), id
        ))
...
def save_project(id=None):
    ...
    project.currency = request.form.get('currency')
