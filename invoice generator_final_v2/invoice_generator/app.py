...
# Import Models
from models import db, Customer, Project, Invoice, PaymentSlab, Service, Contract, ContractSlab, contract_services, CustomerDocument, InvoiceItem
...
def list_customers():
    ...
    if sort_by == 'name_asc': 
        ...
        query += " ORDER BY joined_date DESC" # Default
    ...
    # 4. Render Template
    # We pass 'filter_month' and 'filter_year' because that is what your HTML uses for the 'selected' attribute
...
def create_customer():
    if request.method == 'POST':
        ...
        """, (
            ...
            request.form['currency'], j_date, request.form.get('notes'), request.form.get('tax_disclaimer')
        ))
        conn.commit()
        ...
        if 'documents' in request.files:
            ...
            for file in files:
                if file and file.filename:
                    filename = secure_filename(f"cust_{new_customer_id}_{file.filename}")
                    file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
                    new_doc = CustomerDocument(filename=filename, customer_id=new_customer_id)
...
def edit_customer(id):
    ...
    if request.method == 'POST':
        ...
        cursor.execute("""
            UPDATE customers SET company_name=%s, first_name=%s, last_name=%s, email=%s, phone=%s, 
                ...
                gst_type=%s, gst_number=%s, currency=%s, joined_date=%s, notes=%s, tax_disclaimer=%s
            WHERE id=%s
        """, (
            request.form['company_name'], request.form.get('first_name'), request.form.get('last_name'),
            request.form.get('email'), request.form.get('phone'), request.form.get('address'),
            request.form.get('city'), request.form.get('state'), request.form['country'], 
            request.form.get('deal_owner'), status, tax_type, request.form.get('gst_number'),
            request.form['currency'], j_date, request.form.get('notes'), request.form.get('tax_disclaimer'), id
        ))
        conn.commit()
        conn.close()
        return redirect(url_for('list_customers'))
    ...
    documents = CustomerDocument.query.filter_by(customer_id=id).all()

    return render_template('customer_form.html', customer=customer, documents=documents)

# --- AJAX DOC ROUTES ---
@app.route('/api/upload_document/<int:customer_id>', methods=['POST'])
def upload_document_api(customer_id):
    ...
    if file:
        try:
            ...
            new_doc = CustomerDocument(filename=filename, customer_id=customer_id)
            db.session.add(new_doc)
            db.session.commit()
            ...
            return jsonify({
                ...
                'date': new_doc.upload_date.strftime('%Y-%m-%d'),
                'url': url_for('static', filename='uploads/' + new_doc.filename)
            })
...
def contract_invoice():
    ...
    if request.method == 'POST':
        try:
            ...
            inv_date = datetime.strptime(invoice_date_str, '%Y-%m-%d') if invoice_date_str else date.today()
            ...
            flash("Contract Invoice created successfully!", "success")
            return redirect(url_for('edit_list'))
    ...
    return render_template('invoice_contract_form.html', contracts=contracts, suggested_no=suggested_no)
...
def download_contract_pdf(contract_id):
    ...
    if pisa_status.err: return f"Error creating PDF: {pisa_status.err}", 500
    pdf_stream.seek(0)
...
def edit_service(id):
    ...
    try:
        ...
        service.service_id = request.form['service_id']
        service.description = request.form['description']
        db.session.commit()
        flash(f"Service updated!", "success")
    except Exception as e:
        db.session.rollback()
        flash(f"Error: {str(e)}", "danger")
    return redirect(url_for('manage_services'))
...
def create_project():
    if request.method == 'POST':
        new_project = Project(
            ...
            customer_id=request.form.get('customer_id'),
            # Add other fields as per your Project model and form
        )
    ...
    customers = Customer.query.all()
    return render_template('project_form.html', project=None, customers=customers)
...
def edit_project(id):
    ...
    customers = Customer.query.all()
    return render_template('project_form.html', project=project, customers=customers)
